...
ord_lists = {
    'ws2_32': winsock_ord_list,
    'wsock32': winsock_ord_list,
    'oleaut32': oleaut32_ord_list
}

def get_api_func_name_by_ord(lib_name, ord_num, fullname = False):
    if (len(lib_name) <= 4):
        return None
    lib_name = lib_name.lower()
    if (lib_name[-4:] == '.dll'):
        lib_name = lib_name[:-4]
    ord_list = ord_lists.get(lib_name)
    if (ord_list is not None):
        fnc_name = ord_list.get(ord_num)
        if (fnc_name is None):
            return None
        fnc_name = fnc_name.decode()
    else:
        fnc_name = FUNCNAME_ORD_PREFIX + str(ord_num)
    if fullname:
        return (lib_name + '.' + fnc_name)
    return fnc_name.decode()
